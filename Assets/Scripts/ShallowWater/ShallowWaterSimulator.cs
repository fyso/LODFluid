...
namespace LODFluid
{
	public class ShallowWaterSimulator : MonoBehaviour
	{
		...
		float block_buoyancy;
		float cube_buoyancy;
		Vector3 torque_block;
		Vector3 torque_cube;
		...
		void Start()
		{
			...
			Vector3[] X1 = block.GetComponent<MeshFilter>().mesh.vertices;
			float m = 1;
			block_mass = 0;
			for (int i = 0; i < X1.Length; i++)
			{
				...
				I_ref_block[0, 0] -= m * X1[i][0] * X1[i][0];
				I_ref_block[0, 1] -= m * X1[i][0] * X1[i][1];
				I_ref_block[0, 2] -= m * X1[i][0] * X1[i][2];
				I_ref_block[1, 0] -= m * X1[i][1] * X1[i][0];
				I_ref_block[1, 1] -= m * X1[i][1] * X1[i][1];
				I_ref_block[1, 2] -= m * X1[i][1] * X1[i][2];
				I_ref_block[2, 0] -= m * X1[i][2] * X1[i][0];
				I_ref_block[2, 1] -= m * X1[i][2] * X1[i][1];
				I_ref_block[2, 2] -= m * X1[i][2] * X1[i][2];
			}
			I_ref_block[3, 3] = 1;

			Vector3[] X2 = cube.GetComponent<MeshFilter>().mesh.vertices;
			cube_mass = 0;
			for (int i = 0; i < X2.Length; i++)
			{
				...
				I_ref_cube[1, 1] += diag;
				I_ref_cube[2, 2] += diag;
				I_ref_cube[0, 0] -= m * X2[i][0] * X2[i][0];
				I_ref_cube[0, 1] -= m * X2[i][0] * X2[i][1];
				I_ref_cube[0, 2] -= m * X2[i][0] * X2[i][2];
				I_ref_cube[1, 0] -= m * X2[i][1] * X2[i][0];
				I_ref_cube[1, 1] -= m * X2[i][1] * X2[i][1];
				I_ref_cube[1, 2] -= m * X2[i][1] * X2[i][2];
				I_ref_cube[2, 0] -= m * X2[i][2] * X2[i][0];
				I_ref_cube[2, 1] -= m * X2[i][2] * X2[i][1];
				I_ref_cube[2, 2] -= m * X2[i][2] * X2[i][2];
			}
			I_ref_cube[3, 3] = 1;

			ShallowWaterResources = new ShallowWaterBuffer(new Vector2Int(size, size));
		}
		...
		void cubeWaterCoupling(GameObject block, float[,] new_h, ref float buoyancy, ref Vector3 torque, Vector3[] X)
		{
		...
		public class ShallowWaterBuffer
		{
			...
			public ComputeBuffer OldHeightBuffer;

			public ComputeBuffer NewHeightBuffer;


			public ShallowWaterBuffer(Vector2Int vResolution)
			{
				HeightBuffer = new ComputeBuffer(vResolution.x * vResolution.y, sizeof(float));
				OldHeightBuffer = new ComputeBuffer(vResolution.x * vResolution.y, sizeof(float));
				NewHeightBuffer = new ComputeBuffer(vResolution.x * vResolution.y, sizeof(float));
			...
			~ShallowWaterBuffer()
			{
				...
				OldHeightBuffer.Release();
				NewHeightBuffer.Release();
			}
		}
	}
}
